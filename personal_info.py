...
class Personal_analysis:
    ...
    def get_school_prob(self,df, key_word='', which_day=0):
        '''
        ...
        count_day = 0
        last_day = ''
        if key_word in df['start_address_name'].iloc[0] or key_word in df['end_address_name'].iloc[0]:
            count_day += 1
        ...
        for i in range(1, df.shape[0]):
            ...
            if df['datetime'].iloc[i] == last_day:
                continue
            if key_word in df['start_address_name'].iloc[i] or key_word in df['end_address_name'].iloc[i]:
                count_day += 1
    ...
    # 将个人信息的GPS经纬度转换为地址
    def transform_gps_data(self, filename):
    ...
    #根据经纬度获取地址信息。
        for i in range(df.shape[0]):
            ...
            if pd.isnull(work_address_gps_by_kde_lat):
                continue
    ...
    #     work_address_gps_by_dbscan =None
            ...
            if home_address_gps_by_kde != None:
                ...
                if res_json_home['result']['poiRegions'] != []:
                    df['home_tag'] = res_json_home['result']['poiRegions'][0]['tag']
                else:
                    df['home_tag'] = ''
            ...
            if work_address_gps_by_kde != None:
                ...
                if res_json_work['result']['poiRegions'] != []:
                    df['work_tag'] = res_json_work['result']['poiRegions'][0]['tag']
                else:
                    df['work_tag'] = ''
                df['work_address'].iloc[i] = res_json_work['result']['sematic_description']
            logging.info('已完成%d%%' %((i+1)/df.shape[0]*100))
        df.to_excel(filename)
        logging.info('写入文件：%s完成' % (filename))
    ...
    def get_person_info(self, df_, personalDataPath=None):
        """
        获取用户信息，并进行保存
        ...
        columns = ['user_id', 'car_id',
                   ...
                   # 'provnce', 'city', 'district', 'town', 'street',
                   # #'home_tag',
                   # 'home_address',
                   # #'work_tag',
                   # 'work_address',
                   ...
                   # 'first_address',
                   # 'home_address_guesss',
                   # 'home_address_guesss_process',
                   # 'work_address_guesss',
                   # 'work_address_guesss_process',
                   ...
                   'home_address_weekdend_gps_by_kde_lng', 'home_address_weekdend_gps_by_kde_lat',
                   'work_address_weekdend_gps_by_kde_lng', 'work_address_weekdend_gps_by_kde_lat',
        ...
        nameList = list(df_['user_id'].unique())
        nu_ = len(nameList)
        ...
        for name in nameList:
            ...
            if car_driving_day < 30:
                ...
                start_datetime = None;
                end_datetime = None;
                day_between_start_end = None
                usage_rate_day = None;
                workdaySum_dring = None;
                weekendSum_dring = None
                usage_rate_workday = None;
                usage_rate_weekend = None
                workdaySum = None
                weekendSum = None;
                rate = None
                # province=None; city=None; region=None;
                ...
                # provnce=None; city=None; district=None; town=None; street=None
                # #home_tag=None;
                # home_address=None
                # #work_tag=None;
                # work_address=None
                ...
                home_address_gps_by_kde_lng = None;
                home_address_gps_by_kde_lat = None
                work_address_gps_by_kde_lng = None;
                work_address_gps_by_kde_lat = None

                home_address_weekdend_gps_by_kde_lng = None
                home_address_weekdend_gps_by_kde_lat = None
                work_address_weekdend_gps_by_kde_lng = None
                work_address_weekdend_gps_by_kde_lat = None
                diff_home_lng = None;
                diff_home_lat = None;
                diff_work_lng = None;
                diff_work_lat = None
                ...
                primary_school_prob = None
                mid_school_prob = None
                kindergarten_prob = None
                ...
                save_msg.loc[row_index] = [user_id, car_id, car_driving_time,
                                           ...
                                           home_address_weekdend_gps_by_kde_lng,
                                           home_address_weekdend_gps_by_kde_lat,
                                           work_address_weekdend_gps_by_kde_lng,
                                           work_address_weekdend_gps_by_kde_lat,
                                           diff_home_lng, diff_home_lat, diff_work_lng, diff_work_lat,
                ...
                row_index += 1
                logger.info("读取个人数据：%s....进度%0.2f" % (user_id, row_index / nu_))
                continue

            car_driving_time_everyday = round(car_driving_time / car_driving_day, 2)
            start_datetime = df['datetime'].iloc[0]
            end_datetime = df['datetime'].iloc[df.shape[0] - 1]

            start_datetime_ = pd.to_datetime(df['datetime'].iloc[0])
            end_datetime_ = pd.to_datetime(df['datetime'].iloc[df.shape[0] - 1])
            ...
            home_address_weekdend_gps_by_kde_lng, home_address_weekdend_gps_by_kde_lat = gps_poi_analysis.get_home_address_gps_by_KDE(
                df, weekday=1)
            work_address_weekdend_gps_by_kde_lng, work_address_weekdend_gps_by_kde_lat = gps_poi_analysis.get_work_address_gps_by_KDE(
                df, weekday=1)

            diff_home_lng = abs(home_address_gps_by_kde_lng - home_address_weekdend_gps_by_kde_lng)
            diff_home_lat = abs(home_address_gps_by_kde_lat - home_address_weekdend_gps_by_kde_lat)

            diff_work_lng = abs(work_address_gps_by_kde_lng - work_address_weekdend_gps_by_kde_lng)
            diff_work_lat = abs(work_address_gps_by_kde_lat - work_address_weekdend_gps_by_kde_lat)
            ...
            #
            # provnce='';city='';district=''; town='';street=''
            # home_tag='';home_address=''
            # work_tag='';work_address=''
            # if home_address_gps_by_kde != None:
            #     res_json_home = home_address_gps_by_kde
            #     provnce = res_json_home['result']['addressComponent']['province']
            #     city = res_json_home['result']['addressComponent']['city']
            #     district = res_json_home['result']['addressComponent']['district']
            #     town = res_json_home['result']['addressComponent']['town']
            #     street = res_json_home['result']['addressComponent']['street']
            #     tag = ''
            #     if res_json_home['result']['poiRegions'] != []:
            #         home_tag = res_json_home['result']['poiRegions'][0]['tag']
            #     home_address = res_json_home['result']['sematic_description']
            #
            #
            # if work_address_gps_by_kde != None:
            #     res_json_work = work_address_gps_by_kde
            ...
            #     work_address = res_json_work['result']['sematic_description']
            ...
            primary_school_prob = self.get_school_prob(df,key_word='小学', which_day=1)
            mid_school_prob = self.get_school_prob(df,key_word='中学', which_day=1)
            kindergarten_prob = self.get_school_prob(df,key_word='幼儿园',which_day=1)
